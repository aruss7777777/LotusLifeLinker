...
struct SevenPlayerSplit: View {
    ...
    private func playerInfoView(name: String, life: Int, color: Color, isOutlined: Bool, rotation: Angle) -> some View {
        VStack(spacing: 6) {
            OutlinedPlayerText(
                text: "\(life)",
                font: PlayerTextSizing.lifeFont(baseSize: 64),
                color: color,
            ...
            ZStack {
                ...
                ForEach(1..<4, id: \.self) { row in
                    Rectangle()
                        ...
                        .position(x: geometry.size.width / 4, y: geometry.size.height * CGFloat(row) / 4)
                }
                ...
                ForEach(1..<3, id: \.self) { row in
                    Rectangle()
                        .fill(Color.black.opacity(0.42))
                        .frame(width: geometry.size.width / 2, height: 8)
                        .position(x: geometry.size.width * 3 / 4, y: geometry.size.height * CGFloat(row) / 3)
                }
